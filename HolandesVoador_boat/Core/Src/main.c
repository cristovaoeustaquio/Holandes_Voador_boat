...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	...
	if(((check != NULL)&&(status == INSTRUCTION)) || ((check2 != NULL)&&(status == SCAN))){
		...
		int8_t *rssi = RSSI(buffer,rssis);
		Point coord = get_position(*rssi,*(rssi+1),*(rssi+2));
		snprintf(msg,sizeof(msg)-1,"rssi: %d, %d, %d ** x = %d y = %d\r\n",*rssi,*(rssi+1),*(rssi+2),(int)coord.x,(int)coord.y);
